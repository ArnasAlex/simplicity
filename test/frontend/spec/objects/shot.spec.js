define(["require", "exports", '../helpers/mockFactory', '../../../../frontend/js/app/objects/shot', '../../../../frontend/js/app/objects/enemy'], function (require, exports, MF, SH, E) {
    describe('Shot', function () {
        it('should be created when player shoots', function () {
            var scene = MF.MockFactory.getScene();
            scene.drawArea.mouseLocation = { x: 5, y: 5 };
            var shots = _.filter(scene.objects, function (x) { return x instanceof SH.Shot; });
            expect(shots.length).toEqual(0);
            scene.player.shoot();
            shots = _.filter(scene.objects, function (x) { return x instanceof SH.Shot; });
            expect(shots.length).toEqual(1);
            scene.player.shoot();
            shots = _.filter(scene.objects, function (x) { return x instanceof SH.Shot; });
            expect(shots.length).toEqual(2);
        });
        it('should be removed when it hits enemy', function () {
            var scene = MF.MockFactory.getScene();
            ...
            var enemy = _.filter(scene.objects, function (x) { return x instanceof E.Enemy; })[0];
            var enemyInitialHP = enemy.getHP();
            scene.player.shoot();
            var shot = _.filter(scene.objects, function (x) { return x instanceof SH.Shot; })[0];
            shot.proceed(1);
        ...
        it('should be removed when it reaches max distance', function () {
            var scene = MF.MockFactory.getScene();
            scene.drawArea.mouseLocation = { x: 100, y: 100 };
            scene.player.shoot();
            var shot = _.filter(scene.objects, function (x) { return x instanceof SH.Shot; })[0];
